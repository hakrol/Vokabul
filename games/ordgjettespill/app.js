const WORD_LISTS =
    window.VokabulWordSets?.ordgjettespill?.wordLists || {};
...
const RECORD_KEY = STORAGE_KEYS
    ? STORAGE_KEYS.ORD_GUESS_RECORD
    : "ordgjettespill_rekord";
const RECORD_METRIC = "longestStreak";
const NEXT_DELAY_MS = 1000;

const STATS_TODAY_KEY = STORAGE_KEYS
    ? STORAGE_KEYS.ORD_GUESS_TODAY
    : "ordgjettespill_stats_today";
const STATS_ALLTIME_KEY = STORAGE_KEYS
    ? STORAGE_KEYS.ORD_GUESS_ALLTIME
    : "ordgjettespill_stats_alltime";
const ATTEMPT_KEY = STORAGE_KEYS
    ? STORAGE_KEYS.ORD_GUESS_ATTEMPTS
    : "ordgjettespill_attempts";
...
function getRecord() {
    ...
    if (storage && storage.readJson) {
        return storage.readJson(RECORD_KEY, null);
    }
...
function saveRecord(value) {
    ...
    if (storage && storage.writeJson) {
        storage.writeJson(RECORD_KEY, { longestStreak: value });
        return;
    }
...
function recordAttempt(word, correct, skipped) {
    if (!storageAvailable || !storage || !storage.appendToList) return;
    if (!ATTEMPT_KEY) return;
...
function handleAnswer(isSkip) {
    ...
    recordAttempt(word.ord, isCorrect, isSkip);
